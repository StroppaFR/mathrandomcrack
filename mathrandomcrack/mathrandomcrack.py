...
def recover_state_from_math_random_doubles(doubles, positions=None):
    ...
    for i, double in enumerate(doubles):
        ...
        known_bits[i] = [None for _ in range(12)] + int64_to_bits(v8_from_double(double))[12:HALF_STATE_SIZE]
    # Recover possible states from known bits
    for math_random in recover_state_from_math_random_known_bits(known_bits, positions):
        yield math_random

def recover_state_from_math_random_scaled_values(rounded_vals, factor, translation=0, positions=None):
    """
    Recover all the possible MathRandom states given a list of generated values using Math.round(Math.random() * factor + translate).
    ...
    Arguments:
        ...
        factor, translation: the integers used in the expression Math.round(Math.random() * factor + translation)
    ...
    for i, rounded_val in enumerate(rounded_vals):
        ...
        # FIXME this should be (rounded_val - translation) instead of (rounded_val - translation - 1) but it only work that way and I can't find out why :'(
        low, high = v8_from_double((rounded_val - translation - 1)/ factor), v8_from_double((rounded_val - translation + 1) / factor)
        ...
        known_bits[i] = [None for _ in range(64 - len(common_known_bits))] + common_known_bits
    # Recover possible states from known bits
    for math_random in recover_state_from_math_random_known_bits(known_bits, positions):
        yield math_random
